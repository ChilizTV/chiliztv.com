# PredCast — Functional Documentation
...
- [1. What is PredCast?](#1-what-is-predcast)
...
**PredCast** is a sports entertainment platform built on the **Chiliz Chain** that combines **sports betting** and **live-streaming monetization** into one unified experience.
...
PredCast solves these problems by building on blockchain, where:
...
The Chiliz Chain is purpose-built for sports and entertainment. It hosts **40+ official fan tokens** from major clubs worldwide. PredCast gives those tokens a real purpose — not just collectibles, but active tools for betting and creator support.
...
```mermaid
flowchart LR
    User["🏃 Fan"] -->|"Bets with CHZ,\nfan token, or USDC"| Platform["⚙️ PredCast"]
...
```mermaid
flowchart LR
    Fan["🎉 Fan"] -->|"Donates or subscribes\nwith any token"| Platform["⚙️ PredCast"]
...
1. A fan opens PredCast and selects a football or basketball match.
...
1. A fan watches a sports streamer on PredCast.
...
PredCast gives **real, active utility** to fan tokens. Holders of FC Barcelona, PSG, Juventus, or any other club's token can:
...
| **Multi-chain deployment** | Ready | PredCast can be deployed to other blockchains (Base, Arbitrum, etc.) with minimal changes |
...
flowchart TB
    ...
    subgraph Platform["⚙️ PredCast Platform"]
...
PredCast is not just another betting platform. It is:
...
PredCast's betting system uses a **shared liquidity pool model**: a single ERC-4626 vault holds all USDC. Match contracts hold no funds of their own. This is what makes the platform trustworthy — bettors can verify on-chain that the pool can cover every accepted bet before the bet is recorded.
...
PredCast is a **two-sided platform**: it needs both **streamers creating content** and **viewers engaging** (betting, donating, subscribing). Building this ecosystem requires visibility within the Chiliz community.

**What we need:**

Support from Chiliz in amplifying PredCast to the fan token community:

| Ask | Impact |
|-----|--------|
| **Social media co-promotion** | Announce PredCast on Chiliz official channels to reach the existing fan token holder base |
| **Streamer onboarding** | Help connect us with sports content creators who are already active in the Chiliz/Socios ecosystem |
| **Event visibility** | Feature PredCast at Chiliz ecosystem events, partner showcases, or fan token campaigns |
| **Ecosystem listing** | Include PredCast in the official Chiliz ecosystem page / app directory |
...
PredCast gives **real utility to fan tokens** — not just holding or trading, but active engagement through betting and streamer support. This aligns directly with Chiliz's mission to increase fan token usage. With Chiliz visibility, we can bootstrap the user base needed to make the platform self-sustaining.

...
///         receive transferable ERC-4626 shares (`ctvLP`). The pool earns
///         from losing stakes — bettors pay no placement fee.
/// @dev    Economic model:
///         - Bettors pay 0% on placement.
///         - On each market resolution, losing stakes split:
///             * `treasuryShareBps` (default 40%, capped at 50%) → treasury
///               (accrued; pulled via `withdrawTreasury`).
///             * remainder (default 60%) stays in the pool and compounds
///               into LP NAV automatically.
///         - LP exits (`withdraw` / `redeem`) pay `lpWithdrawalFeeBps`
///           (default 5%, capped at 20%) on **gain** above tracked
///           `costBasis`. Loss-only exits are fee-free. Performance fees
///           accrue to the same `accruedTreasury` claim as the loss split.
///
///         NAV model:
...
///
///         Cost basis: tracked per holder. Incremented in `_deposit` by the
///         asset amount paid in. Decremented proportionally in `_withdraw`
///         (`cb * shares / preBurnBalance`). On share transfer, proportional
///         cost basis follows the shares to the recipient — keeps the
///         performance-fee math honest under arbitrary share movement.
///
    ...
    ILiquidityPool
{
    ...
    /// @notice Hard ceiling on the treasury's share of losing stakes. Even an
    ///         admin cannot raise the cut above this — bounds rug risk against
    ///         LPs. Today's default is 40% (`treasuryShareBps`); the ceiling
    ///         is 50% to keep the LP-yield majority structural.
    uint16 public constant TREASURY_SHARE_BPS_MAX = 5_000;

    /// @notice Hard ceiling on the LP withdrawal performance fee (20%).
    uint16 public constant LP_WITHDRAWAL_FEE_BPS_MAX = 2_000;
    ...
    /// @notice DEPRECATED — bettors pay no placement fee in the current model.
    ///         Storage slot retained for upgrade-safe layout. The auto-getter
    ///         and the `setProtocolFeeBps` admin function are kept for
    ///         backwards compatibility with the `ILiquidityPool` interface
    ///         and any off-chain consumers; the value is no longer read on
    ///         the bet path.
    ...
    /// @notice Treasury's share of every losing stake, in basis points.
    ///         Default 4_000 (40%) at initializer; admin-tunable up to
    ///         `TREASURY_SHARE_BPS_MAX` (50%). The remainder stays in the
    ///         pool and compounds into LP NAV.
    uint16 public treasuryShareBps;

    /// @notice Performance fee skimmed from each LP's *gain* at withdrawal,
    ///         in basis points. Default 500 (5%) at initializer; admin-tunable
    ///         up to `LP_WITHDRAWAL_FEE_BPS_MAX` (20%). Gain is measured
    ///         against the LP's tracked `costBasis`. No fee on flat or losing
    ///         exits.
    uint16 public lpWithdrawalFeeBps;

    /// @notice Per-holder USDC cost basis. Incremented on every deposit/mint
    ///         by the asset amount paid in, decremented proportionally on
    ///         every withdraw/redeem and on share transfers out. Drives the
    ///         performance-fee math at withdrawal time.
    mapping(address holder => uint256) public costBasis;

    /// @dev Reserved for upgrades. Shrunk from 40 → 38 after appending two
    ///      new storage slots above. `treasuryShareBps` + `lpWithdrawalFeeBps`
    ///      pack into a single slot (two uint16); `costBasis` consumes one
    ///      slot for the mapping root. Do NOT reorder or delete named slots.
    ...
    event LpWithdrawalFeeBpsSet(uint16 oldBps, uint16 newBps);
    ...
    function initialize(
        ...
        // Economic-model defaults. 40% of every losing stake accrues to
        // treasury; 60% stays in the pool as LP NAV yield. LP exits pay a 5%
        // performance fee on gain (against tracked cost basis). Both are
        // admin-tunable up to their hard ceilings.
        treasuryShareBps         = 4_000;
        lpWithdrawalFeeBps       = 500;
        ...
        emit TreasuryShareBpsSet(0, 4_000);
        emit LpWithdrawalFeeBpsSet(0, 500);
    }
    ...
    /// @dev Track last-receive time for cooldown AND migrate cost basis on
    ///      share-to-share transfers so the performance-fee math at the
    ///      recipient's withdrawal time stays honest.
    ///
    ///      Mint (`from == 0`): cost basis is incremented by the asset amount
    ///      in `_deposit` — nothing to do here.
    ///      Burn (`to == 0`): cost basis is decremented proportionally in
    ///      `_withdraw` BEFORE this hook fires — nothing to do here either.
    ///      Transfer between holders: move proportional cost basis from
    ///      sender to recipient. Pre-update sender balance is `balanceOf(from) + value`.
    function _update(address from, address to, uint256 value)
        internal
        override(ERC20Upgradeable)
    {
        if (from != address(0) && to != address(0) && value > 0) {
            uint256 fromCb  = costBasis[from];
            if (fromCb > 0) {
                uint256 fromBalPre = balanceOf(from); // still pre-update at this point
                uint256 moved = (fromCb * value) / fromBalPre;
                if (moved > 0) {
                    costBasis[from] = fromCb - moved;
                    costBasis[to]  += moved;
                }
            }
        }
    ...
    /// @dev On deposit/mint, record the asset amount paid in as `costBasis`
    ///      for the receiver. Both `deposit()` and `mint()` route through
    ///      this OZ hook.
    ...
    /// @dev On withdraw/redeem, peel the proportional slice of the owner's
    ///      cost basis, compute the gain (assets above cost), skim the
    ///      performance fee (`lpWithdrawalFeeBps`) to `accruedTreasury`, and
    ///      deliver the net to `receiver`. Loss-only exits pay no fee.
    ///
    ///      We do NOT call `super._withdraw` because it transfers `assets`
    ///      verbatim — instead we replicate its allowance + burn + transfer
    ///      flow with the fee netted out.
    function _withdraw(
        address caller,
        address receiver,
        address owner,
        ...
        // Cost basis slice consumed by these shares. Pre-burn balance is
        // `balanceOf(owner)` because we haven't burned yet.
        uint256 ownerBal = balanceOf(owner);
        uint256 cb       = costBasis[owner];
        uint256 cbConsumed;
        if (ownerBal > 0 && cb > 0) {
            cbConsumed = (cb * shares) / ownerBal;
            costBasis[owner] = cb - cbConsumed;
        }

        // Performance fee on gain only. Loss exits are fee-free.
        uint256 fee;
        if (assets > cbConsumed) {
            uint256 gain = assets - cbConsumed;
            fee = (gain * lpWithdrawalFeeBps) / BPS_DENOMINATOR;
            if (fee > 0) {
                accruedTreasury += fee;
                emit LpWithdrawalFeeAccrued(owner, gain, fee);
            }
        }

        _burn(owner, shares);
        SafeERC20.safeTransfer(IERC20(asset()), receiver, assets - fee);
    ...
    ///         pay out).
    ///      2. Accrue `treasuryShareBps` (default 40%) of the losing net-stake
    ///         total to the treasury as a pull-based claim. The remainder
    ///         stays in the USDC balance and compounds into LP NAV
    ...
    function settleMarket(
        ...
        if (losingNetStake > 0) {
            uint256 share = (losingNetStake * treasuryShareBps) / BPS_DENOMINATOR;
            if (share > 0) {
    ...
    /// @notice Set the treasury's cut of losing stakes. Bounded by
    ///         `TREASURY_SHARE_BPS_MAX` (50%) so an admin compromise cannot
    ///         flip the LP-yield majority.
    function setTreasuryShareBps(uint16 newBps) external onlyRole(DEFAULT_ADMIN_ROLE) {
        if (newBps > TREASURY_SHARE_BPS_MAX) revert BpsOutOfRange(newBps, TREASURY_SHARE_BPS_MAX);
        emit TreasuryShareBpsSet(treasuryShareBps, newBps);
        treasuryShareBps = newBps;
    }

    /// @notice Set the LP withdrawal performance fee. Bounded by
    ///         `LP_WITHDRAWAL_FEE_BPS_MAX` (20%). Applied only to gain over
    ///         tracked `costBasis` on `withdraw` / `redeem`.
    function setLpWithdrawalFeeBps(uint16 newBps) external onlyRole(DEFAULT_ADMIN_ROLE) {
        if (newBps > LP_WITHDRAWAL_FEE_BPS_MAX) revert BpsOutOfRange(newBps, LP_WITHDRAWAL_FEE_BPS_MAX);
        emit LpWithdrawalFeeBpsSet(lpWithdrawalFeeBps, newBps);
        lpWithdrawalFeeBps = newBps;
    }

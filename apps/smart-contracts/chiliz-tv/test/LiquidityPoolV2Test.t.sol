...
contract LiquidityPoolV2Test is Test {
    ...
    // ═══════════════════════════════════════════════════════════════════════
    // LOSS SPLIT (treasuryShareBps to treasury / remainder to LPs)
    // ═══════════════════════════════════════════════════════════════════════

    function test_LossSplit_AccruesShareToTreasury_RemainderToLPs() public {
        ...
        // Post-resolve expectations (split is treasuryShareBps / 10_000 of stake):
        //   accruedTreasury = 100 × treasuryShareBps / 10_000
        //   LP NAV          = 10_000 + (100 − accruedTreasury)
        uint256 shareBps = uint256(pool.treasuryShareBps());
        uint256 expectedAccrued = (100e6 * shareBps) / 10_000;
        uint256 expectedLpYield = 100e6 - expectedAccrued;
        assertEq(pool.accruedTreasury(), expectedAccrued,                   "treasury accrual");
        assertEq(pool.totalAssets(),     10_000e6 + expectedLpYield,        "LP NAV up by remainder");
    ...
    function test_LossSplit_MixedBetsOnlyAccrueLosingStakes() public {
        ...
        // Only bob's 300 losing netStake accrues; alice's win is unaffected.
        uint256 expected = (300e6 * uint256(pool.treasuryShareBps())) / 10_000;
        assertEq(pool.accruedTreasury(), expected, "accruedTreasury = share of losing only");
    }
    ...
    function test_FreeBalance_ExcludesAccruedTreasury() public {
        _seedPool(10_000e6);
        _accrueTreasury(500e6); // helper computes the stake at the live share rate

        uint256 free = pool.freeBalance();
        uint256 ta   = pool.totalAssets();
        assertEq(free, ta, "freeBalance mirrors totalAssets");

        // accruedTreasury is 500. Losing stake at the live share rate is
        // 500 × 10_000 / treasuryShareBps; LP yield = stake − accrued. Pool
        // USDC = 10_000 seed + losingStake. LP NAV = USDC − accrued.
        uint256 losingStake = (500e6 * 10_000) / uint256(pool.treasuryShareBps());
        uint256 lpYield     = losingStake - 500e6;
        assertEq(free, 10_000e6 + lpYield, "LP NAV = seed + LP yield");
        assertEq(pool.accruedTreasury(), 500e6, "treasury = configured share of losing");
    }
    ...
    ///      losing bet whose stake produces `target` at the current
    ///      `treasuryShareBps` rate. Stake-to-target ratio is read live, so
    ///      the helper survives future admin tweaks of the share.
    ...
    function _accrueTreasury(uint256 target) internal {
        ...
        uint256 shareBps    = uint256(pool.treasuryShareBps());
        uint256 losingStake = (target * 10_000) / shareBps;
        uint256 marketId = _newMarketAtOdds(20_000);

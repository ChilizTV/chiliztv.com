...
contract SwapIntegrationTest is Test {
    ...
    // ══════════════════════════════════════════════════════════════════════════
    // REGRESSION: router-placed bets must reach the LiquidityPool, not the
    // BettingMatch. Pre-fix, ChilizSwapRouter._placeBetOnBehalf transferred
    // USDC to the bettingMatch — leaking stake funds. This test does NOT
    // pre-fund the pool so the leak would surface as a balance mismatch.
    // ══════════════════════════════════════════════════════════════════════════

    function test_RouterBet_USDCRoutedToPoolNotMatch_Regression() public {
        // Tear down setUp's pool pre-funding so the only USDC at the pool
        // comes from a real LP deposit + the bet stake itself.
        address sink = address(0xDEAD);
        uint256 prefund = usdc.balanceOf(address(pool));
        vm.prank(address(pool));
        usdc.transfer(sink, prefund);
        assertEq(usdc.balanceOf(address(pool)), 0, "pool drained for the test");

        // Charlie acts as a real LP — deposits 1 000 USDC into the pool.
        vm.startPrank(charlie);
        usdc.approve(address(pool), 1_000e6);
        pool.deposit(1_000e6, charlie);
        vm.stopPrank();
        assertEq(usdc.balanceOf(address(pool)), 1_000e6, "pool seeded by LP");

        // Open a 2.00x WINNER market.
        vm.prank(owner);
        footballMatch.addMarketWithLine(MARKET_WINNER, 20_000, 0);
        vm.prank(owner);
        footballMatch.openMarket(0);

        // Alice places a 100 USDC bet via the router (USDC fast-path).
        vm.startPrank(alice);
        usdc.approve(address(swapRouter), 100e6);
        swapRouter.placeBetWithUSDC(address(footballMatch), 0, 0 /* Home */, 100e6);
        vm.stopPrank();

        // The stake MUST be on the pool, NOT the match contract.
        assertEq(usdc.balanceOf(address(footballMatch)), 0,        "match holds no USDC");
        assertEq(usdc.balanceOf(address(pool)),          1_100e6,  "pool holds LP + stake");

        // Resolve as a WIN for alice. Pool pays 200 USDC (2.00x payout).
        vm.prank(owner);
        footballMatch.closeMarket(0);
        vm.prank(resolver);
        footballMatch.resolveMarket(0, 0);
        ...
        assertEq(usdc.balanceOf(alice) - aliceBefore, 200e6,       "alice wins 200 USDC payout");
        assertEq(usdc.balanceOf(address(footballMatch)), 0,        "match still holds 0 USDC");
        assertEq(usdc.balanceOf(address(pool)),          900e6,    "pool balance: 1000 + 100 - 200");
    }

    function test_RouterBet_LosingBet_FundsCompoundIntoPool_Regression() public {
        // Same setup as the win regression: drain pre-funding, real LP deposit.
        address sink = address(0xDEAD2);
        uint256 prefund = usdc.balanceOf(address(pool));
        vm.prank(address(pool));
        usdc.transfer(sink, prefund);
        ...
        // Alice bets 100 USDC on Home.
        vm.startPrank(alice);
        usdc.approve(address(swapRouter), 100e6);
        swapRouter.placeBetWithUSDC(address(footballMatch), 0, 0, 100e6);
        vm.stopPrank();

        // Resolve as Away — alice loses.
        vm.prank(owner);
        footballMatch.closeMarket(0);
        vm.prank(resolver);
        footballMatch.resolveMarket(0, 1);

        // 40% of losing stake → treasury (default), 60% stays in pool as LP yield.
        uint256 expectedAccrued = (100e6 * uint256(pool.treasuryShareBps())) / 10_000;
        assertEq(pool.accruedTreasury(), expectedAccrued,                 "40% of losing stake to treasury");
        assertEq(usdc.balanceOf(address(pool)),          1_100e6,          "pool retains stake");
        assertEq(usdc.balanceOf(address(footballMatch)), 0,                "match still holds 0 USDC");
        assertEq(pool.totalAssets(),                     1_100e6 - expectedAccrued, "LP NAV reflects 60% yield");
    }
}
